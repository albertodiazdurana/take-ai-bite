...
- **BACKLOG-537** (High): Central's own gitignored `.claude/commands/` copies **shadow** the user-level ones and nothing keeps them current, so `dsm-review-feedback.md` went 56 lines stale the instant BL-533 edited the source , the fix was invisible at the hub that shipped it. Two hand-repairs in six sessions is the signal to change the mechanism.
