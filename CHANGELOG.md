...
- **The always-loaded DSM_0.2 core drops from 3,278 lines to 1,873, a reduction of 1,405 (BL-543 arms A and B).** Four siblings were extracted: **`DSM_0.2.E_Provenance.md`** (the Origin blocks, which are read at most once per rule and were being loaded on every turn of every session), **`DSM_0.2.F_Pre_Generation_Brief.md`** (§8 in full), **`DSM_0.2.T_Alignment_Templates.md`** (the §17.1 base template text `/dsm-align` copies verbatim), and **`DSM_0.2.G_Transcript_Enforcement.md`** (§7's enforcement narrative, origins and worked examples). The core keeps the behavioural rules and points at each module; the two operational rules that are behaviour rather than narrative, the turn-boundary self-check and `[RETROACTIVE]` recovery, stay stated in §7 itself rather than moving with the narrative. The extraction was originally built as a mass-reduction experiment against a different hypothesis, which was disproven; it is kept on its own merits, since the always-loaded budget is spent on every turn regardless of what the experiment concluded.
  **Spoke action:** None beyond ensuring the four new module files are present alongside the core. Spokes reading Central directly get them automatically; mirror-based spokes receive them in this version's sync.

- **Hub operations moved out of the always-loaded config into `dsm-docs/guides/dsm-central-operations.md`.** The Change Tracking, Version Update, Change Propagation and wrap-up-checklist workflows were carried in Central's `CLAUDE.md`, which is loaded in full on every turn, to be read on perhaps four occasions: a wrap-up, a release, a mirror sync, or a backlog operation. `CLAUDE.md` now names those four triggers and points at the guide.
  **Spoke action:** None. Central-only.

### Fixed

- **A wording pattern in the always-loaded reinforcement block correlated with a model-side session-start refusal on newer models (BL-543).** Sessions on the newest model family began failing at boot. The cause was **semantic and highly local**: one imperative phrasing in the Session Transcript *reinforcement* block, in `.claude/CLAUDE.md` and in the §17.1 base template, read to a model-side safeguard as a request of a kind it declines. It was **not** a size or threshold problem, and the evidence for that is the part worth keeping: three successive context reductions, one of them below a size already measured as clean, all still failed, and removing that single block cleared it. Localized by **manual ablation** after the mass hypothesis had been pursued and disproven. The fix rewords the block to the protocol's own native vocabulary, work notes and plan, which is what the block always meant.
  **Spoke action:** Run `/dsm-align`. **This is the only entry in this release that requires it:** the reword lands inside the §17.1 base template, so a spoke's reinforcement block keeps the old phrasing until it realigns.
  A standing practice comes with it and is stated here because it outlives the fix: the phrasing that triggers a model-side safeguard must never be written into a file that is loaded at session start, since the memory and handoff machinery propagates recorded phrases into exactly those surfaces, which is how this one reproduced itself across sessions. Verbatim detail is quarantined to a research log that no boot reads. The robustness follow-up is filed as BACKLOG-545.

- **`/dsm-go` and `/dsm-light-go` counted nine scaffold folders where the specification and the scaffolder both say eight (BL-534).** A correctly scaffolded project reported `8/9` at every boot and could never reach its own stated maximum. The gate never misfired, which is exactly why it survived: a number that is wrong in a way that looks like a known quirk gets written off. Five consecutive spoke reports plus one hub boot did write it off. It had also **propagated**: a spoke was measured carrying an empty `dsm-docs/inbox/.gitkeep` created to satisfy the phantom entry, which inflated its count to a false `9/9`. The threshold is unchanged at five folders and only the denominator moves, which makes the gate strictly tighter for any project that had grown the ghost folder.
  **Spoke action:** Run `scripts/sync-commands.sh --deploy`.

- **The mirror manifest could not carry the new module files (mirror sync scope).** Category 1's pattern read `DSM_0.2.[A-D]_*.md`, so the character class itself excluded E, F and G, and `.T` belonged to no letter range at all. The documentation described a range while the sync list enumerated paths, and the range is the half that reads as coverage. A by-hand run of the bidirectional check the manifest still defers found **six** absent files rather than the four expected; the census is recorded in the manifest's maintenance notes, because the count is the only evidence that the manual discipline does not hold on its own. The general rule the measurement produced: a dangling reference costs nothing until the file *containing* it is mirrored, so the owed set is (references) intersected with (synced files), never all references.
  **Spoke action:** None. Affects mirror repos only.

- **The transcript delimiter is now `Start Plan`, and the per-turn reminder was the first thing changed (BL-545).** The BL-543 fix reworded the always-loaded reinforcement block but left the delimiter inside it reading `Start Thinking`, so the vocabulary the fix moved away from was still present on every load. The rename covers 13 occurrences across 8 live files, together with the prose that named the block. Two findings reordered the work and are recorded because the backlog item said otherwise: the reminder injected on **every turn** lives in `.claude/hooks/transcript-reminder.sh`, not in `.claude/settings.json` as filed, and it is a higher-frequency carrier of the label than the always-loaded document, where the string sits once inside a large file. It was changed first, alone, in its own commit.
  Nothing needed to change in the validator's logic and nothing needs migrating: check 3 matches the general `<------------Start ` form rather than a list of type names, and it inspects only the content being appended, so an existing transcript is never re-validated. Both labels validate, measured in both directions. The two names a rename would wrongly absorb were checked and left alone: `Critical Thinking` is a DSM principle, and the transcript header's `**Thinking:**` field names a harness toggle.
  **Spoke action:** Run `/dsm-align`, the same run the BL-543 entry above already requires. The delimiter label lands in the §17.1 template, so a spoke keeps the old one until it realigns, and a spoke that never realigns still validates, since both labels pass.
...
Minor rather than patch: BL-531 and BL-520 each add a new DSM_0.2 section, which is the "New DSM section or methodology track" row of DSM_2.0.D §7.4's cadence table. The "3+ backlog improvements" row would give a patch; the higher trigger wins.

### Added

- **DSM_0.2 §21.4: BL testing validates the change and never the artifact it lands in (BL-531).** §21.2 makes a BL name its failure modes at filing time, §21.3 makes the implementer execute its Test Plan with per-item evidence, and §19 sets the minimum for a task branch. All three are scoped to **one BL**, so all three can pass while the artifact the change landed in is left internally inconsistent, and nothing asked about the artifact as a whole. The new check asks one question , is this artifact still internally consistent with itself , at the **Resolver** unit, the term `dsm-docs/plans/done/INDEX.md` already maintains for the section, skill or concept a BL landed as. Its mechanical half is a sweep for every string the change removed or renamed, where **a hit is a candidate and never a verdict**, since a deliberate legacy reference and a stale one look identical to `grep`. Its prose half is three checks the sweep cannot make: cross-references resolve, asserted counts are recomputed rather than carried forward, and the artifact's own file is actually carried by whatever distributes it. It **reports and does not gate**, and it satisfies §8.9.2's over-firing guard structurally rather than by tuning, because a change that renamed nothing produces an empty sweep and is silent on exactly the ordinary work a noisy gate would train the reader to dismiss. The axis against its siblings is stated so the rules do not read as a pile: §19.1/§19.2/§19.3 govern verification **quality**, §21.3 and §21.4 govern verification **obligation**, and within obligation §21.3 is the change unit while §21.4 is the Resolver unit. Origin: six BLs shipped in one session, the user asked whether they interfered, they did not, and **nothing in DSM required anyone to check**; the audit that ran only because it was asked for found a fourth instance of BL-521's class at `DSM_3.0.C:265`.
  **Spoke action:** Review §21.4 before the next multi-BL session. Reaches spokes through the `@` chain; no `/dsm-align` needed for this entry.

- **DSM_0.2 §19.3: a delegated workflow's returned result can be a stub while every upstream stage succeeded (BL-520).** §19.1 governs snippets published in skill files and §19.2 the ad-hoc commands an agent composes in the moment; both ask their question at the shell grain. §19.3 asks it at the **delegation grain**, where nothing previously checked whether a payload survived the return trip. The origin run completed 5 angles, 19 sources, 70 claims, 25 verified and 0 refuted across 101 agents and roughly 3.1M tokens, then returned `summary: "test"` with a single finding reading `"test claim"`. Three properties compound and are why this needed its own section: every upstream signal reports success, the stub is **syntactically valid** so schema validation cannot catch it, and the cost is already sunk when it arrives. Hence a plausibility comparison against the run's own reported stats rather than a schema check or an absolute size floor, and a four-rung recovery ladder whose ordering is the rule, because the first two rungs are free: the workflow's own output file, then **the per-agent transcripts**, then re-synthesis of the failed stage alone, and only then a re-run. The transcript path in the BL as filed was wrong and the implementation says so: they live under `subagents/`, verified on this machine, and the section names the role rather than the path because harness layouts move.
  **Spoke action:** Review §19.3 before the next background-workflow or fan-out run. Reaches spokes through the `@` chain; no `/dsm-align` needed for this entry.

- **A project-level plan tier, and a ruling on its filename (BL-519).** Eleven planning templates existed across DSM_2.0.A/B/C and the finest-grained one carrying a full structure was Template 8, Sprint Plan; above it there was nothing, so a project arriving with a complete decomposition already done had no canonical home for the whole-project view. **DSM_2.0.C Template 13** fills it, and the §17.1 base template gains one line routing such a project through `dsm-docs/plans/PROJECT-PLAN.md` rather than re-decomposing its source, since re-decomposing produces two descriptions of the same ladder. The filename is ruled **`PROJECT-PLAN.md` with a hyphen**: DSM_0.1's anti-pattern table said underscore, the one real usage in the ecosystem used a hyphen, neither was normative, and a third spoke would have picked a third spelling. Numbered 13 and not the vacant 9 on purpose , filling 9 would silently change what "Template 9" means in three live documents that reference the vacancy. The template changed **because its test was run**: tracing the one real 25,750-byte instance found its header reads `Supersedes:`, so the source does **not** stay the authority there, contradicting the draft's central reference-don't-copy rule and revealing a second case the draft was missing.
  **Spoke action:** Run `/dsm-align`. **This is the only entry in this release that requires it:** the routing line lands inside the §17.1 base template, so a spoke's reinforcement block otherwise lacks it entirely.
...
- **Two commands were never mapped to the mirror, and closing the gap trips an invisible staging trap (BL-528).** `scripts/commands/` held 20 sources against 18 `source >> target` mapping lines; `dsm-research-add` and `dsm-research-done` had **never** been in the mapping block, so a fresh mirror clone received 18 of 20 deploy copies and category 6 of the manifest silently failed for two of them. The more useful half is why nobody noticed: the obvious reading, that the mirror has no copy, is wrong. The files were present and byte-identical, so `diff -q` matched, the sync loop hit `continue`, and the copy branch was never reached , identical content is not delivered content, and the first implementation of the fix reproduced exactly that defect before `sync-take-ai-bite.sh --check` caught it. Running the shipped tool rather than reading the diff is what found it.
  **Spoke action:** None. Mirror-side manifest change; take-ai-bite receives it through the sync.

- **`FEATURES.md` paired a maintained number with an unmaintained one (BL-529).** The header read "160 features across 11 capability domains". The first figure was enumerated and sound; the second was inherited from a 2026-03-16 inventory covering **84** features, so roughly half the published total had never been classified against the taxonomy the sentence invoked. The claim may well still have been true , nothing here asserts a twelfth domain , and the defect is that one sentence presented a maintained and an unmaintained number with the same authority. The domain clause is dropped rather than recomputed, because the mirror ships the claim and, by manifest design, can never ship its evidence.
  **Spoke action:** None.
...
- **BACKLOG-539** (High): DSM governs the approval surface of an autonomy grant thoroughly and its **size** not at all; agent success decays exponentially in task length and the distribution is memoryless, so progress within a run is not evidence about the next step. Filed with a two-pass research base.
...
- **DSM_0.2 §8.9.2: consent priced on N agents could run 2N, and the trigger excluded the mitigation the section recommends (BL-524).** Two defects found by the spoke that adopted the gate. **Recursive delegation:** nothing stopped a subagent inside an approved fan-out from spawning a fan-out of its own, observed as a six-agent approval running eleven, and the whole section contained one occurrence of "delegat", not about this. Closed as a fail-safe default rather than a sixth disclosure: a gated subagent has **no delegation authority** unless the accepted prompt granted it, and where granted, disclosure 1 states the bound on *total* agents. A sixth disclosure would appear in every prompt including the many where the answer is trivially no, which is the ritualism shape §8.7 and §10.1 both guard against. **The trigger wording:** it read "the same rate-limit pool" while the mitigations recommend moving subagents to a cheaper tier, which on a plan with a separate weekly pool for that tier takes the recommended configuration *out* of the gate as literally written. Resolved as a wording slip, matching the section's own line 1210: the trigger is now "a shared, finite rate-limit pool", where shared means shared among the fan-out's own calls. A fan-out is dangerous because N calls contend for one exhaustible budget; exhausting the cheaper pool stops the work exactly as hard. The over-firing guard is untouched and was verified verbatim, because the obvious way to fix this is to broaden the trigger and breadth is the failure this section is best at avoiding.
  **Spoke action:** Review §8.9.2 before the next multi-agent run. Reaches spokes through the `@` chain; no `/dsm-align` needed for this entry.

- **Three Sprint and Phase boundary checklist items named paths nothing creates (BL-521).** They failed in opposite directions and both directions produce a checklist whose ticks do not mean what they appear to. Item 5 carried `or N/A if no smoke tests recorded this sprint`, true forever of a project that will never record them, so the check could not fire; it is now scoped to a project property, matching item 4's existing `(DSM 4.0 projects)` form. Item 7 named `dsm-docs/blog/README.md`, which no scaffolding step created, so it could not be ticked truthfully at all; `/dsm-align` Step 3 now creates it from a Blog Publication Tracker template stating that an empty table is a valid state meaning nothing has been published, which is different from the file being absent. The audit that the BL required as its third part did **not** return "no others": the Phase Boundary Checklist's first item pointed at `methodology.md`, a file `/dsm-align` itself flags as legacy and asks spokes to migrate off, so a spoke on the current per-session format had nothing to update. Fixed in the same commit, since auditing for a class and then shipping a known instance of it is worse than not auditing.
  **Spoke action:** Run `scripts/sync-commands.sh --deploy`. The next `/dsm-align` creates `dsm-docs/blog/README.md` if it is missing; existing files are never overwritten.

- **`/dsm-quick-wrap-up` appended reasoning lessons without regenerating their compact mirror (BL-514).** Three sibling steps write `.claude/reasoning-lessons.md` and only two maintained the mirror the next boot reads, with no stated reason for the asymmetry. Quick wrap-up now regenerates it, referencing DSM_0.2.A §8.1's canonical transform rather than re-inlining a copy, re-inlining being the BL-447 drift bug that silently emptied the mirror on flat-structured spoke files. Deliberately recorded as a *placement* fix rather than a data-loss one: no lesson was ever lost, because `/dsm-go` Step 1.5 detects a stale mirror by mtime and regenerates it inline before reading. What this moves is a ~96 KB regeneration off the boot path, where context is scarcest, onto the wrap-up path. Six sites in DSM_0.2.A that asserted a two-regenerator count were corrected alongside; the BL named two of them, and a grep found eight.
  **Spoke action:** Run `scripts/sync-commands.sh --deploy`.

### Spawned

- **BACKLOG-528** (Medium): `dsm-research-add` and `dsm-research-done` have never been in the mirror manifest's mapping block, so a fresh mirror clone receives 18 of 20 deploy copies; and closing that gap trips a staging trap invisible from the checkout, since one of the two ignore rules lives in `.git/info/exclude`.
- **BACKLOG-529** (Low): `FEATURES.md` claims "160 features across 11 capability domains", where the domain figure is inherited from a 2026-03-16 inventory covering 84. The mirror ships the claim and, by manifest design, can never ship its evidence.
- **BACKLOG-530** (Medium): filed and implemented in this release; see Added.
- **BACKLOG-531** (High): DSM tests the unit of change and never the unit of integrity; §21.2, §21.3 and §19 are all scoped to one backlog item, and all three can pass while the artifact the change landed in is left internally inconsistent. Filed thin, with the naming question and the success criteria deliberately open.

## [1.23.0] - 2026-08-20

### Added

- **DSM_0.2 §20.4: a PR is built from the remote ref, so verify the branch is fully pushed before opening one (BL-516).** `gh pr create` and every equivalent describe the branch as it exists **on the remote**. A commit made after the last push is absent from the PR, absent from the merge, and then destroyed when `--delete-branch` removes the only copy holding it, with every step reporting success. §20.4 now requires `git rev-list @{u}..HEAD --count` to be `0` before a PR is created or merged, and halting with the commit subjects rather than pushing silently, since an unpushed commit at PR time usually means the branch is not in the state its author believes. It also records a measurement that changes the shape of the remedy: **git already implements this guard and it works.** `git branch -d` refuses a branch holding a commit not in its base, exit 1; `git branch -D` force-deletes. Because `gh pr merge --delete-branch` does delete such branches, it uses force semantics, so the check must sit **before** the `gh` call and can never rely on the tool declining. Placed inside §20.4 rather than as a new §20.9, because §20.4's subject is exactly when and why a branch is pushed and a new subsection would be a methodology section addition; §20 still has 8 subsections. `/dsm-wrap-up` gains Step 9.4 carrying **only** the executable check plus a pointer to §20.4, so the rationale lives in exactly one place, the restate-and-drift hazard this two-site shape invites was verified closed at 1 reference and 0 copies. Origin: a session committed a README fix after pushing its branch, opened the PR from the stale ref, merged, and lost the commit; it was recovered from the reflog and noticed only because a later step diffed the file and found no change where there plainly should have been one.
  **Spoke action:** Run `scripts/sync-commands.sh --deploy`, then review DSM_0.2 §20.4. The check runs at wrap-up; no `/dsm-align` needed, the §17.1 template is unchanged.
- **DSM_0.2.A §8.1: the provenance-header rule gains a create case (BL-526).** BL-510's three properties are all update-shaped and presuppose the `**Last appended:**` line already exists: there is no superseded value to chain on a first run, and the one stated skip condition is about content rather than about the line being absent. An agent meeting an absent line had three readings, create, skip, error, and the rule distinguished none of them. Property 4 now states it: create the line when absent, and say **within the line** that this run created it and that earlier appends predate the header, so its date is not later misread as the file's first write. The property is deliberately two-sided, because "create if absent" read carelessly is a false provenance claim in the other direction: absence is not a reason to skip, and not a reason to write either, property 2's no-op still governs. The gap is not hypothetical, since BL-510 post-dates many spokes' last structural edit to the file, so a spoke that has not restructured since then has no header at all. Propagated to all four sites and verified byte-identical across the three restatements by md5 rather than by presence.
  **Spoke action:** Run `scripts/sync-commands.sh --deploy`. A spoke whose `.claude/reasoning-lessons.md` has no `**Last appended:**` line will now have one created at its next wrap-up, correctly labelled.

### Fixed

- **Five slash commands deployed where no spoke could see them, and the drift check reported them healthy (BL-518).** `sync-commands.sh` split its command set, sending `dsm-backlog`, `dsm-backlog-done`, `dsm-checkpoint`, `dsm-review-feedback` and `dsm-version-update` to `${REPO_DIR}/.claude/commands`, a path only a session running inside Central resolves. Spokes fell through to user-level copies that **no code path wrote**: measured at four months stale, mtimes 2026-03-20 and 2026-04-22, all five differing from source. The consequence was not staleness in the abstract. The spoke-visible `/dsm-backlog` carried **zero** occurrences of `Risks` and **zero** of `Test Execution Log` against two each in source, so every spoke filing a backlog item through it produced one that violated DSM_0.2 §21.2 and §21.3, the two rules Central shipped to make backlog items auditable, while the agent believed it had followed the skill. It also wrote to the legacy `plan/backlog/` tree. `/dsm-checkpoint` shipped spokes the pre-F-146 template with the entire Causal-Forward section missing. Throughout, `--check` reported `20/20 | OK: 20` and was not lying: it compared each command against its **own designated target**, and for those five that target was Central's folder, which was current. The file a spoke actually loads was outside the comparison. Resolved as universal deployment, the split was never a real category, since these commands are used in every project. All 20 now deploy user-level, and `--check` additionally reports orphaned copies at the legacy path, which **shadow** the user-level ones and would otherwise go stale invisibly; they are reported and never deleted, since removing substantive files is the repository owner's call. The mirror manifest gained the five `source >> target` mappings in the same commit as its guide, without which a mirror would have kept the stale shadowing copies and reproduced the defect one level down. Same silent-success family as BL-482, BL-488 and BL-512, and reported by a spoke rather than found here.
  **Spoke action:** Run `scripts/sync-commands.sh --deploy`. **This is the entry with a real consequence:** until it runs, the project keeps loading a four-month-stale `/dsm-backlog` that omits the Risks and Test Execution Log sections, and a `/dsm-checkpoint` missing Causal-Forward. Any backlog item filed through the old copy should be checked for those two sections.
- **The session baseline holds two disagreeing records of untracked files, and the wrap-up did not say which it compared against (BL-523).** `/dsm-go` Step 5 writes `# Working tree` from a plain `git status --porcelain`, which collapses a wholly-untracked directory to a single entry, and writes `# Checksums` with `-uall` on the untracked line, which enumerates it. `/dsm-wrap-up` Step 9's first classification bullet said "files not in the baseline" without naming which block, and asserted that Step 5 "writes the baseline with the same flag", which is true of one of its three invocations. Measured on a baseline holding one loose file and one untracked directory with two files at two depths, the two readings return **opposite verdicts on the same files**: compared against `# Working tree` both nested files are staged as new-this-session, compared against `# Checksums` both are correctly skipped as pre-existing. The dangerous direction is the first, which sweeps prior uncommitted work into the session's commit. Both wrap-up skills now name the `# Checksums` block as the comparison surface and `/dsm-go` says so at the write site, since nothing downstream can tell which block an agent consulted. Deliberately **not** done: propagating `-uall` to the neighbouring lines to make the flags "agree", which is the obvious wrong fix the false sentence invited and which BL-512's reasoning explicitly forbids. Fourth member of the BL-482 / BL-488 / BL-512 family, all in the same four lines, all silent at execution, all found by spokes. Testing also corrected `/dsm-light-go`, which pointed at "`/dsm-go` step 6" for a baseline written at step 5.
  **Spoke action:** Run `scripts/sync-commands.sh --deploy`. Deploy `dsm-go.md`, `dsm-wrap-up.md`, `dsm-quick-wrap-up.md` and `dsm-light-go.md` together; a baseline written under one convention and read under another is the failure this removes.
- **`/dsm-align`'s report idempotence invariant excluded one header line where four can vary (BL-527).** The persistent report's invariant read "the report content (excluding the timestamp line) is identical". The header carries five fields and three of them can legitimately differ between two runs over an unchanged project: the timestamp always, the DSM version on any release, and the run mode between a post-change run and a check-only run. A conforming run therefore looked like a violation. The invariant is now stated over the report **body**, bounded by the `---` separator the report already writes rather than by a field list that would drift as fields are added.
  **Spoke action:** Run `scripts/sync-commands.sh --deploy`.

### Spawned

- **BACKLOG-519** (Medium): no project-level plan template exists above DSM_2.0.C Template 8, and the filename DSM_0.1 prescribes (`PROJECT_PLAN.md`) disagrees with the only real usage (`PROJECT-PLAN.md`), so a third spoke will pick a third spelling. Two independent spokes reached the same absence.
- **BACKLOG-520** (Medium): a background workflow's returned result can be a placeholder stub while every upstream stage reports success, so consent-priced work is discarded and the stub passes schema validation.
- **BACKLOG-521** (Medium): two Sprint Boundary Checklist items name paths no scaffold creates, failing in opposite directions, one permanently waivable through an `or N/A` clause, one impossible to satisfy at all.
- **BACKLOG-522** (Low): `dsm-docs/guides/` has no index and no convention for trigger-gated references, so a lesson that is a landmine under one setup and noise everywhere else has nowhere to live.
- **BACKLOG-524** (Medium): §8.9.2's cost gate does not survive recursive delegation (a six-agent approval ran eleven), and its trigger says "the same rate-limit pool" while its own mitigation recommends moving to a separate one.
- **BACKLOG-525** (Medium): the Sprint Boundary Checklist exists in two places with no authority rule, and in the one observed case neither list was a subset of the other, so the obvious reconciliation would have deleted two legitimate local practices.

## [1.22.0] - 2026-08-20

### Added

- **DSM_0.2 §19.2: a pipeline's exit status belongs to its last command (BL-515).** A shell pipeline exits with the status of its *last* command, so `cmd | tail && echo ok` and `cmd | wc -l || echo none` test `tail` and `wc`, which essentially always succeed. The failing command's status is discarded before anything looks at it. The section states both failure directions, which look nothing alike and are usually noticed separately: false success, where a guard fires on a command that failed, and false emptiness, where a fallback stays silent and its silence reads as "ran fine". It names the three correct forms (`$?` alone, `set -o pipefail`, `${PIPESTATUS[0]}`), each verified to surface an exit 128 the defective form swallowed, and prohibits `| tail` as a success probe while explicitly leaving `| tail` for display alone. Sibling to §19.1 rather than an amendment to it: §19.1 governs snippets *published* in skill files, §19.2 governs the ad-hoc verification commands an agent composes mid-turn, which appear in no file and are reviewed by nobody. A corpus sweep confirms the split empirically at zero published instances, against a positive control matching 3 of 3 on a synthetic file. What promoted this from a reasoning lesson to a rule is recurrence under active awareness: S247 ran `git push | tail && echo pushed` and reported "pushed to main" on a push the protected branch had **rejected**, in a session whose own transcript already carried the lesson. Placed in DSM_0.2 core, which is always loaded via the `@` chain, so it reaches every spoke without costing the §17.1 base template a line, verified byte-identical rather than merely intended to be untouched.
  §19.2 carries a clause added during this same release: the display carve-out (bounding noisy output with `| tail -25` is legitimate) covers the pipe and **not** a later `$?`. That clause exists because the session that wrote §19.2 committed the defect within the hour, running the mirror-sync content scanner through `| tail -25` and reporting "scanner exit: 0" when the scanner had exited 1 and its own output said "Mirror sync should stop". Recorded in the section's Origin rather than quietly patched, since a rule its own author broke on day one is the strongest evidence for why it needed to exist.
  **Spoke action:** Review DSM_0.2 §19.2 for behavioral changes. No `/dsm-align` needed; the §17.1 template is unchanged.

### Fixed

- **Both stated byte caps on the compact reasoning-lessons mirror were unenforceable, in opposite directions (BL-491).** `/dsm-go` Step 1.5 advertised a hard cap of 8 KB while instructing the agent to read the file **in full**, and justified the instruction on that premise; the file measured 105,855 B, false by 12.9x, and had been false for months with nothing reporting it. DSM_0.2.A §8.1's advisory cap of `max(60 KB, 1.2 × live file size)` could not be exceeded by any strict-trim transform, since the mirror is always smaller than its source, so it returned `OK` on a file sitting at 1.72x the degrade threshold that same section names. One bound now, stated once, in §8.1: **61,440 B**. It is derived rather than restated, §8.1's own pre-existing >60 KB degrading-priming-load threshold and the measured 62,494 B that one `Read` call returns before truncating agree within 1.7%, which is also why the bound is not simply today's file size. The breach observable is specified per §19.1's second question: regeneration reports measured size, bound and ratio in every case, so a pass and a breach differ in verdict rather than in silence. Demonstrated against a constructed breach case with the old formula as a negative control, returning an identical `OK` for a 40,000 B mirror and a 105,855 B one.
  **Spoke action:** Run `scripts/sync-commands.sh --deploy`.
- **`/dsm-go` Step 1.5 no longer claims the compact mirror fits (BL-491, behavioural half).** A truncated `Read` **is** the breach signal, which needs no literal in the skill file and therefore cannot drift from §8.1. On truncation the agent must report the breach with measured size, bound and ratio, and state which reduced read it performed, rather than silently paging through or quietly reading less and continuing. Before this wording a truncated read and a complete one produced identical boot reports. `/dsm-wrap-up` Step 0 now references §8.1 instead of restating the formula, leaving exactly one file in the corpus carrying the value. Deliberately untouched: BL-490's entry-count target, whose exact substring is preserved, and BL-427 Step 5 compression.
  **Spoke action:** Run `scripts/sync-commands.sh --deploy`. Deploy `dsm-go.md` and `dsm-wrap-up.md` together; a boot that reports a breach against a wrap-up that still restates the old formula is the drift this fix removes.

### Spawned

- **BACKLOG-508** (Medium): wrap-up skills delete `.claude/session.lock` at a final step but none checks it on entry, so a stale window can run a 19-step autonomous skill against a live sibling's project and then delete that sibling's lock. Disjoint from DSM_0.2.A §26.7's deferred foreign-append check, which asks the expensive question; this is the cheap one `/dsm-go` Step 0.7 already answers. Filed with its central design question deliberately open and marked not agent-resolvable: whether detection halts outright or reuses Step 0.7's three-way prompt, and what a force flag would mean for a wrap-up whose steps include a merge to main.
...
- **BL-387: Permission-system parity for push-to-main vs PR-merge-to-main.** Closes an asymmetry where `git push origin main` required user confirmation but `gh pr merge {N} --merge|--squash|--rebase` to a main-base PR did not, even though both operations have identical publication outcome. DSM_0.2.C §2 gains a new "PR-merge to main" bullet in the Destructive Action Protocol's "Operations requiring confirmation" list, documenting the equivalence with an explicit "equivalent in publication outcome, not equivalent in policy enforcement" nuance (PR merge respects branch protection; direct push bypasses it). Confirmation may be batched within a larger work-block approval but must be specific to the merge, not absorbed into a general "proceed" instruction. DSM_0.2.C §2.2 is a new subsection alongside §2.1 documenting the opt-in permission-rule pattern (`Bash(gh pr merge:*)` deny pattern in `.claude/settings.json`) for security-sensitive projects, honest that it matches all PR merges, not just to main, with When-To-Adopt and When-NOT-To-Adopt criteria. DSM Central's project CLAUDE.md Destructive Command Protocol section gains a back-reference bullet pointing to DSM_0.2.C §2 + §2.2 for visibility. Couples tightly with BL-386: together they form a target-verify → action-authorize chain for destructive-merge operations.
  - **Spoke action:** Review DSM_0.2.C §2 PR-merge bullet + §2.2 for the new equivalence rule and opt-in permission pattern. Projects that want Option A hardening can copy the §2.2 pattern into their `.claude/settings.json`; project CLAUDE.md back-reference is not mirrored (each project decides whether to adopt).

- **BL-380: `/dsm-backlog` creation-time sprint-plan template injection.** `scripts/commands/dsm-backlog.md` gains a new Step 2.5 that detects sprint-plan intent (title regex `^Sprint\s+\d+\b` case-insensitive OR `--sprint` flag) and injects a Sprint Plan Template Scaffold containing the 5 DSM_2.0.C §1 Template 8 sections (Research Assessment, Deliverables, Phases, Phase Boundary Checklist, Sprint Boundary Checklist) plus the 3 header-block fields (Duration, Goal, Prerequisites). `--no-template` flag suppresses injection even on sprint titles (escape hatch). Scaffold-only approach (headings + 1-line placeholders pointing to DSM_2.0.C §1 for authoritative content) avoids drift if Template 8 evolves. Non-sprint BL creation unchanged. Paired with BL-378's detective-side `/dsm-align` Step 3a audit: BL-380 prevents the omission at creation time; BL-378 catches plans created outside `/dsm-backlog` or on unconventional sprint titles. Origin: S193 inbox triage thread T3.
  - **Spoke action:** Run `sync-commands.sh --deploy` to pick up the new Step 2.5 + flag handling. Behavior change is opt-in (only triggers on sprint-titled BLs or `--sprint` flag); non-sprint BL creation is unaffected.

### Spawned

- **BL-388 (Medium):** Hook-Block Rollback Opacity. Observation from S193/S194 STAA: when the `git mv` rename-staging pre-commit hook blocks a commit, the retry preserves only what is named in the fix command and silently drops staged content changes from the index, forcing a Part 2 commit to restore the missed edits.
- **BL-389 (Medium):** STAA Skill Scope Audit and Documented-Behavior Alignment. Observation from S194 STAA: the `/dsm-staa` skill file documents a narrow scope ("does NOT modify any project files except `.claude/reasoning-lessons.md`"), but observed behavior extends to MEMORY promotion and BL filing. Documented-vs-actual scope mismatch to reconcile.
...
- **BL-373 F2: `.claude/commands/*.md` (13 user-level) now shipped tracked in mirror clones.** Closes the cross-platform bootstrap gap discovered in BL-372 T7 where fresh clones on Windows PowerShell could not invoke `/dsm-go` because `sync-commands.sh --deploy` required bash + correct HOME resolution. Mirrors now carry the commands in `.claude/commands/` as tracked files; Claude Code reads project-level commands automatically on first clone, with zero bash dependency. Enhancement to `scripts/sync-take-ai-bite.sh` supports a new `source >> target` mapping syntax so `scripts/commands/<cmd>.md` (Central source of truth) ships to `.claude/commands/<cmd>.md` (mirror runtime). Central's `.gitignore` line 117 unchanged: runtime deploy copies on Central remain gitignored (regenerated by `sync-commands.sh --deploy`). On mirrors, first sync requires `git add -f` to override the inherited gitignore rule; once tracked, the files stay tracked across future syncs. 18 commands total: 13 user-level shipped via this mechanism; 5 project-level already shipped via the existing sync list (unchanged behavior for those).
  - **Spoke action:** None required. Mirror sync is a Central-side operation. Fresh TAB clones from v1.5.2 onward have commands available immediately on open; no `/dsm-align` or manual deploy needed before first `/dsm-go`.
