...
- **Compact reasoning-lessons mirror** — Cross-session reasoning lessons are distilled into a file the agent reads at session start; when the file outgrows its size limit the session reports the breach instead of quietly reading less
...
See the full timeline of 165 features → [FEATURES.md](FEATURES.md)
