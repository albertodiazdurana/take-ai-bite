...
|---|------|-------|----|--------|--------------|--------------|
| F-134 | 2026-06-01 | User-Reframes-Proposal Handling sub-rule (DSM_0.2 §8.0.1) | BL-453 | DSM Central reasoning-lessons pattern promotion: six data points (S180/S183/S200/S201/S203/S204) over 4+ months; §8.0 principle present throughout but agents kept defending their framing. | — (internal pattern promotion) | Principle → operational behavior gap: a stated principle ("the conversation IS the collaboration") does not prevent a failure mode when the behavior it implies is unnamed. Sibling cross-reference without absorption (4 adjacent non-yes/no-response patterns). |

### v1.11.0 (2026-05-31)

| F | Date | Title | BL | Origin | Inbox source | Meta-pattern |
|---|------|-------|----|--------|--------------|--------------|
| F-133 | 2026-05-31 | Smoke tests as a named DSM artifact (DSM_4.0.A §7 + DSM_4.0 §4.1 + DSM_2.0.C) | BL-452 | traveline-ds-project-skeleton S1: ~80 lines of per-item validation commands the user ran lived only in transcript context, no canonical place to persist. | `_inbox/done/traveline-ds-project-skeleton.md` | Name the industry-standard practice (smoke test) and give it a repo home rather than coining a term; Gate-1 grounding dropped the proposal's DSM_0.1 §10 edit (folder-only table) and append-not-renumber avoided silent divergence. |
| F-132 | 2026-05-31 | Collision-safe dated inbox-done filenames (DSM_0.2 §17.1 + CLAUDE.md) | BL-451 | DSM Central S211 self-detected §22 incident: `mv` overwrote a 347-line rolling `done/` archive (−323 lines, recovered pre-commit). The exact collision recurred LIVE during S212 boot inbox processing, validating the fix in the same session that shipped it. | — (internal incident) | Make the safe path the default: dated filenames make the natural `mv` non-colliding by construction, instead of relying on per-agent discipline to append. Recursive validation (bug recurs in fixing session). |
| F-131 | 2026-05-31 | /dsm-wrap-up Step 0.5 pre-confirm cross-repo targets | BL-450 | blog-poster S22 (auto-memory MEMORY.md trips the BL-391 gate every wrap-up) + DSM Central S210 checkpoint Pending #7 (manual mid-flow resolution) + S206-STAA promotion candidate. | `_inbox/done/` blog-poster S22 | Cross-repo-gate-satisfied-by-protocol-named-action carve-out (S193/S194/S204). Live-test of the shipped bash caught a `find\|head-1` wrong-project bug → deterministic slug derivation. |
| F-130 | 2026-05-31 | Transcript replace_all guard (DSM_0.2 §7 + validate-transcript-edit.sh) | BL-449 | Two independent spoke incidents: IronCalc S17 (transcript reached 95 MB / 1.5M lines via `replace_all`) + blog-poster S22 (`replace_all` duplicated an Output block at every match). | IronCalc S17 + blog-poster S22 inbox | Append-only-anchor assumption made enforceable: a deterministic hook Check 0 + a named recovery path beats a prose-only warning. Two-spoke confirmation. |
| F-129 | 2026-05-31 | Cross-repo writes are write-only (DSM_0.2.C §2) | BL-448 | DSM Central S211: a concurrent ds-portfolio session committed `c9d4b83` on Central's active session branch and bundled Central's staged checkpoint move into the foreign commit. | — (internal incident) | Rule-with-rationale (foreign-commit + staged-work-bundling + lock-bypass race), hook deferred per §21. Protocol-rule-first converts a silent deviation into a named §22 violation. |

**Why this matters for users:** F-129 through F-132 are one coherent silent-failure / filesystem-discipline hardening release. Each closes a way a session could silently corrupt state: a foreign git op on another repo's active branch (F-129), a `replace_all` transcript explosion (F-130), repeated cross-repo confirmation friction that pushed agents toward risky shortcuts (F-131), and a `mv` that overwrites a rolling archive (F-132). Two were two-spoke-confirmed (F-130) or recurred live in the fixing session (F-132); all four shipped with per-item Test Execution Logs.

### v1.9.0 (2026-05-05)

| F | Date | Title | BL | Origin | Inbox source | Meta-pattern |
|---|------|-------|----|--------|--------------|--------------|
| F-128 | 2026-05-05 | Chunked Drafting Protocol (DSM_0.2 §8.10) | BL-430 | haystack-magic S8 R3 hiring challenge (2026-05-04). Two structured documents (~2565 + ~3519 words) generated by full-file Write after Gate 2 outline approval. The first carried a load-bearing factual error (misattributed deepset 5-Step Guide labels) that escaped into the SUBMITTED deliverable because per-section review never happened. Source-check question fired hours after submission. The user surfaced the meta-level failure: full-file generation of structured documents defeats the four-gate model's intent for prose. The chunked-drafting feedback file proposing this protocol was itself drafted using the protocol it proposes (recursive validation). | `_inbox/done/2026-05-04_haystack-magic-s7-s8-bls-and-lessons.md` | Take a Bite + four-gate model applied to a missing case. The principle (chunk-by-chunk review) was already explicit for code (function-by-function) and notebooks (cell-by-cell); prose was silent. Naming a case the principle already covers, rather than introducing a new principle, is the lighter codification path. |
| F-127 | 2026-05-05 | Concurrent-Session Detection Protocol (DSM_0.2.A §26 + lockfile) | BL-431 | heating-systems-conversational-ai S10.L2 (2026-04-29). Two Claude Code conversations running in parallel against the same project on the same git branch (`session-10/2026-04-23`). Neither agent detected the other. Discovery happened only when `git status` and the shared `.claude/session-transcript.md` showed entries the active agent had not authored. Both sessions on the same branch, modifying the same working tree, appending to the same transcript, with no awareness of each other. | `_inbox/done/2026-04-29_heating-systems-conversational-ai-s10-s10l2-s11.md` | Single-session-per-window invariant made structural via lockfile. Every other DSM protocol (transcript continuity, baseline integrity, MEMORY pointer, branch state) depends on the invariant; this BL is the first that ENFORCES it. Parallel-session protocol (commit booking) is exempted because parallel sessions are concurrent-by-design siblings, not silent-hazard concurrents. |
| F-126 | 2026-05-05 | Non-Suppressible Prompts Convention (DSM_0.2 §8.9.1) | BL-432 | heating-systems S10.L2. Auto mode silently bypassed `/dsm-go` Step 5.9's light-wrap-up continuation prompt. The agent recognized the prompt's existence in its thinking block ("§5.9 normally offers /dsm-light-go switch, but user explicitly typed /dsm-go, I'll honor that"), then unilaterally pressed past it. The user was never given the choice §5.9 mandates. Combined with the absence of a concurrent-session safety net (BL-431 closed that gap), §5.9 was the only existing user-facing safeguard, and it was swallowed. | `_inbox/done/2026-04-29_heating-systems-conversational-ai-s10-s10l2-s11.md` | Auto-mode-boundary family extended: §8.9 already protected Gate 1 / Gate 3 / §19/§21.3 testing; §8.9.1 closes the procedural-safety-prompt gap. Same anti-pattern guard shape as §8.2.1 / §21.2 / §21.3 / §8.9. The user's explicit suspension does NOT extend to non-suppressible prompts, the explicit boundary against rationalizing-the-skip. |
| F-125 | 2026-05-05 | Skill Scope Is Authoritative principle (DSM_0.2 §8.6.1) | BL-435 | heating-systems S10 (2026-04-23). The agent ran a correctly-invoked `/dsm-align` on a spoke; Step 11 was correctly skipped by the spoke guard. The agent then ran an out-of-scope `diff -q` on user-scope command files and folded the finding into /dsm-align's `Command sync:` report field as `Drifted: 2` plus a fabricated spoke-action recommendation. The drift finding was real but it was NOT /dsm-align's output; it was an off-skill check folded into the skill's report. ~115 transcript lines of correction. | `_inbox/done/2026-04-29_heating-systems-conversational-ai-s10-s10l2-s11.md` | §8.6 catches misremembering; §8.6.1 catches augmentation. Same end-state symptom (skill appears to say something it didn't), different mechanism. Sibling to BL-434 (specific Command-sync wording fix); §8.6.1 generalizes the lesson to all skills. |
| F-124 | 2026-05-05 | /dsm-staa Step 8 regenerate compact reasoning-lessons mirror | BL-433 | haystack-magic S7 STAA continuation (2026-05-02). After /dsm-staa appended 6 [STAA] entries and pruned 2 [auto] entries at ~22:17, the compact mirror remained at its 22:04 state from S7 wrap-up, 13 minutes stale, missing 6 lessons and including 2 pruned ones. The agent regenerated inline manually; this BL makes the regeneration a normative part of /dsm-staa. | `_inbox/done/2026-05-04_haystack-magic-s7-s8-bls-and-lessons.md` | Source-of-truth + derived-mirror separation (cf. F-122) extended to a second mutator. The compact mirror is the boot-time canonical context per `/dsm-go` Step 1.5; both /dsm-wrap-up Step 0 and /dsm-staa Step 8 must regenerate when they touch the live file. The transform implementation in /dsm-staa was caught wrong on first draft (used `## Session N` instead of `### Category` heading match) and fixed via behavioral byte-diff against the current compact mirror, demonstrating the value of comparing against the working artifact rather than against the prose specification. |
| F-123 | 2026-05-05 | /dsm-align Step 12 conditional `Command sync` spec | BL-434 | heating-systems S10 (2026-04-23), same incident as F-125. The blank-field-invites-invention failure mode at the specific `Command sync` field. | `_inbox/done/2026-04-29_heating-systems-conversational-ai-s10-s10l2-s11.md` | Skill-output-discipline at field granularity: spoke runs (Step 11 skipped) emit `N/A (not DSM Central)` verbatim; Central runs emit populated counts. Sibling to F-125 / §8.6.1: F-123 fixes the wording, F-125 generalizes the lesson. Two-BL pair where the specific fix and the principle land together so the principle has a concrete first compliant example. |

### v1.8.0 (2026-04-24)

| F | Date | Title | BL | Origin | Inbox source | Meta-pattern |
|---|------|-------|----|--------|--------------|--------------|
| F-122 | 2026-04-29 | Compact reasoning-lessons mirror (DSM_0.2.A §8.1) | BL-427 | S205 conversation about how routine sessions actually use `.claude/reasoning-lessons.md`. Surfaced that `/dsm-go` Step 1.5 read only the first 10 lines because the live file (50+ KB) was too big to load in full. User reframed the 10-line peek as a context-budget workaround, not §8 design intent. | — (S205 in-session dialog) | Source-of-truth + derived-mirror separation. The same content reaches two different readers (humans + STAA reading the live file; routine agent reading the compact mirror) without forcing one reader's needs onto the other. Implementation measurement (5% savings vs 25-30% projected) was the kind of evidence the controlled-experiment requirement in Step 5 is designed to preserve: design intuitions about "compact" should be tested before scaling. |
