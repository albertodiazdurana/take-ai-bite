...
1.5. **Read reasoning lessons:** If `.claude/reasoning-lessons-compact.md` exists, prefer it (per DSM_0.2.A §8.1). Read the compact mirror and report any category particularly relevant to the current project. The mirror drops guidelines + provenance (target ~70 entry lines), so reading it in full is the design intent **when it fits**.

   **The read is bounded, and a partial read must never be reported as a complete one.** The byte bound lives in DSM_0.2.A §8.1 and is stated there only; do not restate the number here. Operationally you do not need it, because the harness answers the question directly:

   | What the read does | What it means | What to do |
   |---|---|---|
   | One `Read` call returns the whole file | Within bound | Read it in full. Proceed. Say nothing about size. |
   | The call truncates at the token cap | **Breach.** The file has outgrown the instruction | Do NOT silently page through it, and do NOT quietly read less and continue. Report the breach in the boot report with the measured size, the §8.1 bound, and the ratio. Then either read the remainder in paged calls or read the category headings plus the most recent entries, and **state in the report which of the two you did.** |

   Truncation IS the breach signal, so no separate check is needed and no literal can drift. This is DSM_0.2 §19.1's second question applied to a read rather than a snippet: before this wording, a truncated read and a complete one produced identical boot reports, and the agent's own summary of the lessons looked the same either way.

   **Why this replaced "read the compact mirror IN FULL":** the instruction was justified on a premise , that the mirror was size-bounded to something readable in one call , which was false by roughly thirteen times when measured on 2026-08-20, and had been false for months with nothing reporting it. The superseded figure is not repeated here on purpose: it lived in this file while the authoritative bound lived in DSM_0.2.A §8.1, and two copies drifting apart is how the premise stayed false unnoticed. An agent meeting a file that will not fit an instruction saying it must, resolves the contradiction by improvising, and a truncated read reported as a complete one is the failure this wording exists to prevent.
