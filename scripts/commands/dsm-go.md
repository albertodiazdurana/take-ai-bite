...
1.8. **Ensure alignment (critical, auto-run, no confirmation):** Read `.claude/last-align.txt` if it exists.

   **Unconditional rule:** If this step determines that `/dsm-align` must run, invoke it immediately. Do not ask the user. Do not present a y/n confirmation gate. Do not defer to "later in the session". Do not phrase the action as a question ("Should I run /dsm-align?"). The only valid outputs of this step are (a) pass silently, or (b) auto-run `/dsm-align` without any confirmation prompt. Any other response is a protocol violation under DSM_0.2 §22 (Protocol Violation Triage). Rationale: alignment drift silently disables §17.1 template updates across spokes; gating it behind confirmation lets drift persist whenever the user says "not now".

   - **If missing or `result: critical`:** Report the issue in one line, then **auto-run `/dsm-align`** immediately. Do not ask permission. Do not prompt y/n. After `/dsm-align` completes, re-read the updated marker and CLAUDE.md, then continue to step 2 with the fixed state.
   - **If `dsm-version` differs from current DSM version:** Report in one line: "Alignment was run against vX.Y.Z, current DSM is vA.B.C — auto-running /dsm-align." Then **auto-run `/dsm-align`** immediately. Do not ask permission. Do not prompt y/n. After `/dsm-align` completes, re-read the updated marker and CLAUDE.md, then continue to step 2 with the fixed state.
