...
1. **Identify session context:**
   ...
   - Find the BL folder: `docs/plans/BL-*-{descriptor}/`

2. **Update BL README:** Update the `docs/plans/BL-{NNN}-{descriptor}/README.md`:
...
3. **Commit on branch:**
   ```bash
   git add docs/plans/BL-{NNN}-{descriptor}/
...
6. **Append to checkpoint (if exists):**
   Check for an active checkpoint: `ls docs/checkpoints/*.md 2>/dev/null | grep -v done | head -1`
   ...
   **Action required:** Review BL folder contents and distribute to proper locations.
   ...
   ```
   docs/checkpoints/checkpoint-session-N-parallel-{descriptor}.md
   ```
